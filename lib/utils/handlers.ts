...
interface ApiCallParams<I extends ZodTypeAny> {
	// Added constraint here for clarity
	input?: z.infer<I>;
	token?: string;
...
>(options: ApiHandlerOptions<I, O, W, Paginated>) {
	...
	return async ({ input, token, query }: ApiCallParams<I>): Promise<Output> => {
		const {
			method,
			path,
			input: inputSchema,
		...
		if (wrapper === 'standard') {
			...
			validatedData = (isPaginated ? parsed.data : parsed.data.data) as Output;
		} else {
			if (outputSchema) {
		...
		if (transform && 'data' in validatedData) {
			...
			validatedData = (
				isPaginated ? { ...validatedData, data: transformed } : transformed
			) as Output;
		} else if (transform) {
